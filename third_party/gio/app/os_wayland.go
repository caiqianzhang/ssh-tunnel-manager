...
// order of preference.
//
// PORT FIX: added the canonical "text/plain;charset=utf-8" (with hyphen)
// as the first entry. The original list only knew "charset=utf8" (no
// hyphen), so a source offering ONLY the canonical MIME type matched
// nothing and seat.clipboard stayed nil, making every paste silently
// fail until the next focus/selection cycle.
var clipboardMimeTypes = []string{"text/plain;charset=utf-8", "text/plain;charset=utf8", "UTF8_STRING", "text/plain", "TEXT", "STRING"}
...
func (w *window) ReadClipboard() {
	...
	if r == nil || err != nil {
		// PORT FIX (upstream bug): when there is no seat or no pending
		// clipboard selection offer (e.g. the window has not received
		// keyboard focus yet), readClipboard returns nil. Without this
		// reset, readClipClose stays non-nil forever and every future
		// clipboard read in the process silently no-ops — Ctrl+V and
		// programmatic paste stop working until restart. No transfer is
		// in flight on this path, so resetting the guard is safe.
	...
	go func() {
		defer r.Close()
		// PORT FIX: bound the wait for the clipboard source. A stale
		// offer (its source app exited or stopped responding) used to
		// hang io.ReadAll forever, keeping readClipClose non-nil so
		// every later read was silently dropped by the guard above.
		...
		case <-time.After(2 * time.Second):
			// Stale/unresponsive clipboard source: give up so the read
			// guard is released and later pastes still work.
		}
