...
type x11Window struct {
	...
	// Local patch: manual drag state for windows with custom
	// decorations (WM ignores _NET_WM_MOVERESIZE on some compositors).
	dragging           bool
	dragOffX, dragOffY C.int
	...
	atoms struct {
		...
		// "_NET_WM_MOVERESIZE" (interactive window move, ICCCM §4.2.8)
		wmMoveResize C.Atom
...
func (w *x11Window) Configure(options []Option) {
	...
	// Local patch: honor the Decorated option instead of forcing
	// decorations on (see setMotifDecorations below).
	...
	// Local patch: allow undecorated windows (custom title bars) by
	// asking the window manager, via _MOTIF_WM_HINTS, to strip its
	// decorations. Upstream forces decorations on for X11.
	if !cnf.Decorated {
		w.setMotifDecorations(false)
	}
	w.ProcessEvent(ConfigEvent{Config: w.config})
}

// setMotifDecorations toggles WM decorations through the
// _MOTIF_WM_HINTS property (MWM_HINTS_DECORATIONS with an empty
// decoration set removes the title bar and frame).
func (w *x11Window) setMotifDecorations(decorated bool) {
	var data [5]C.long
	data[0] = 2 // MWM_HINTS_DECORATIONS
	if decorated {
		data[2] = 1 // MWM_DECOR_ALL
	}
	C.XChangeProperty(w.x, w.xw,
		w.atom("_MOTIF_WM_HINTS", false), w.atom("_MOTIF_WM_HINTS", false),
...
func (w *x11Window) Perform(acts system.Action) {
	walkActions(acts, func(a system.Action) {
		...
		case system.ActionMinimize:
			// Local patch: custom-decorated windows need the window
			// management actions Gio upstream only wires on other
			// platforms.
			screen := C.XDefaultScreen(w.x)
			C.XIconifyWindow(w.x, w.xw, screen)
...
func (w *x11Window) raise() {
	...
	*ev = C.XClientMessageEvent{
	...
	// Local patch: _NET_ACTIVE_WINDOW requires a source indication in
	// data[0]; an all-zero message is ignored by mutter for windows
	// minimized via XIconifyWindow.
	data := (*[5]C.long)(unsafe.Pointer(&ev.data))
	data[0] = 2 // source indication: pager (mutter ignores 0/1 for
	// deiconification, but honors a pager request)
	data[1] = 0 // timestamp
	data[2] = 0 // requestor's currently active window
	C.XSendEvent(
		w.x,
	...
	// Local patch: an iconified window produces no rendering frames,
	// so nothing else flushes the X output buffer — without an
	// explicit XFlush the message never reaches the WM.
	C.XFlush(w.x)
}
...
// beginDrag starts a manual window drag: remember the offset between
// the window's top-left and the pointer; MotionNotify then repositions
// the window while the primary button stays held.
func (w *x11Window) beginDrag(xroot, yroot C.int) {
	var attrs C.XWindowAttributes
	C.XGetWindowAttributes(w.x, w.xw, &attrs)
	w.dragOffX = xroot - attrs.x
	w.dragOffY = yroot - attrs.y
	w.dragging = true
}

// moveResize asks the window manager to start an interactive move of
// the window, ICCCM _NET_WM_MOVERESIZE with direction 8
// (_NET_WM_MOVERESIZE_MOVE). Used for dragging custom-decorated windows.
func (w *x11Window) moveResize(xroot, yroot C.int) {
	var xev C.XEvent
	ev := (*C.XClientMessageEvent)(unsafe.Pointer(&xev))
	*ev = C.XClientMessageEvent{
		_type:        C.ClientMessage,
		display:      w.x,
		window:       w.xw,
		message_type: w.atoms.wmMoveResize,
		format:       32,
	}
	data := (*[5]C.long)(unsafe.Pointer(&ev.data))
	data[0] = C.long(xroot)
	data[1] = C.long(yroot)
	data[2] = 8 // _NET_WM_MOVERESIZE_MOVE
	data[3] = C.long(C.Button1)
	data[4] = 1 // source indication: application
...
func (h *x11EventHandler) handleEvents() bool {
	...
	for C.XPending(w.x) != 0 {
		...
		case C.ButtonPress, C.ButtonRelease:
			...
			if bevt._type == C.ButtonPress && btn == pointer.ButtonPrimary {
				// Local patch: honor system.ActionMove input ops so
				// windows with custom decorations can be dragged.
				if act, ok := w.w.ActionAt(f32.Point{X: float32(bevt.x), Y: float32(bevt.y)}); ok && act == system.ActionMove {
					w.beginDrag(C.int(bevt.x_root), C.int(bevt.y_root))
					continue
				}
			}
			switch _type {
		...
		case C.MotionNotify:
			...
			if w.dragging {
				C.XMoveWindow(w.x, w.xw,
					C.int(mevt.x_root)-w.dragOffX,
					C.int(mevt.y_root)-w.dragOffY)
				C.XFlush(w.x)
			}
...
func newX11Window(gioWin *callbacks, options []Option) error {
	...
	w.atoms.wmMoveResize = w.atom("_NET_WM_MOVERESIZE", false)
